...
class TodayAppointmentsView(APIView):
    ...
    def get(self, request):
        ...
        today = timezone.now().date()
        appointments = Appointment.objects.filter(doctor=doctor, appointment_date=today).order_by("token_number")
...
class ConsultationPageView(APIView):
    ...
    def get(self, request, appointment_id):
        ...
        current_consultation = Consultation.objects.filter(appointment=appointment).first()
        previous_consultations = Consultation.objects.filter(appointment__patient=patient).exclude(appointment=appointment).order_by("-created_at")[:3]
        previous_prescriptions = Prescription.objects.filter(consultation__appointment__patient=patient).order_by("-created_at")[:3]
        lab_results = LabResult.objects.filter(lab_order_item__lab_order__patient=patient).order_by("-created_at")[:3]

        data = {
            "appointment": AppointmentSerializer(appointment).data,
            "patient": PatientDetailSerializer(patient).data,
            "current_consultation": PreviousConsultationSerializer(current_consultation).data if current_consultation else None,
        ...
        return Response({"message": "Consultation page data fetched successfully", "data": data}, status=status.HTTP_200_OK)
...
class CreateConsultationView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            consultation = serializer.save()
            return Response({"message": "Consultation created successfully", "consultation_code": consultation.consultation_code}, status=status.HTTP_201_CREATED)
...
class CreateLabTestRequestView(APIView):
    ...
    def post(self, request):
        serializer = LabTestRequestSerializer(data=request.data, context={"request": request})
        if serializer.is_valid():
            lab_request = serializer.save()
            return Response({"message": "Lab test request created successfully", "lab_request_id": lab_request.id}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ViewLabResults(APIView):
    ...
    def get(self, request, consultation_id):
        ...
        lab_results = LabResult.objects.filter(lab_order_item__lab_order__lab_request__consultation=consultation)
        serializer = LabResultViewSerializer(lab_results, many=True)
        return Response({"message": "Lab results fetched successfully", "results": serializer.data}, status=status.HTTP_200_OK)
...
class CreatePrescriptionView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            prescription = serializer.save()
            return Response({"message": "Prescription created successfully", "prescription_code": prescription.prescription_code}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)