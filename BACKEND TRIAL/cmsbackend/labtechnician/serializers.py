...
class LabOrderItemSerializer(serializers.ModelSerializer):
    ...
    lab_test_cost = serializers.ReadOnlyField(source='lab_test.cost')  # Add cost for billing
    lab_test_normal_range = serializers.ReadOnlyField(source='lab_test.normal_range')
    lab_test_unit = serializers.ReadOnlyField(source='lab_test.unit')
...
class LabOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LabOrder
        fields = '__all__'
        read_only_fields = ['order_number', 'created_at']  # CRITICAL FIX: Mark auto-generated fields as read-only

    def get_patient_name(self, obj):
        """Include patient name in the response"""
        try:
            patient = obj.patient
...
class LabResultSerializer(serializers.ModelSerializer):
    ...
    def get_patient_name(self, obj):
        """Include patient name in the response"""
        try:
            patient = obj.lab_order_item.lab_order.patient
            return f"{patient.first_name} {patient.last_name}".strip()
        except:
            return None
    ...
    def _sync_order_status(self, lab_order):
        """Auto-complete order when all results are entered"""
...
class LabBillSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LabBill
        fields = '__all__'
        read_only_fields = ['bill_number', 'final_amount', 'created_at']  # CRITICAL FIX
